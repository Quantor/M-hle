...
char connect_search(int i, char board[], int number, Connections edge, int number)
{
	for (int k = 0; k < 64; k++)
	{
		int q, p;
		q = edge[k].Knoten1 - 'A';
		p = edge[k].Knoten2 - 'A';
		if (edge[k].Knoten2 == 'A' + i && board[q] == '.')
		{
			return edge[k].Knoten1;
		}
		if (edge[k].Knoten1 == 'A' + i && board[p]=='.')
		{
		...
		* respective player. */
		...
		if(unplaced_pieces[current_player]==0)
		{
			for (int trys=0; piece_move=' '; trys++)
			{
				...
				if (connect_search(i, board, 24, edge, 64))
				{
					piece_move = connect_search(i, board, 24, edge, 64);
					piece_put = 'A' + i;
				}
			}
		}
